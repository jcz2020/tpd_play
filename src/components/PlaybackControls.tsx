  ...
  return (
    <Card className={cn(!isDeviceOnline && "bg-muted/50")}>
      <CardHeader>
        ...
        <CardDescription>
          {isDeviceOnline ? `Now playing on ${device.ip}` : "Device is offline"}
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col items-center space-y-6">
        ...
        <div className="text-center w-full">
          <h3 className="text-2xl font-semibold tracking-tight">{track.title}</h3>
        ...
        <div className="flex items-center justify-center space-x-4 w-full">
          <Button variant="ghost" size="icon" disabled={!isDeviceOnline}>
            <Rewind className="h-6 w-6" />
          </Button>
          <Button size="lg" className="rounded-full w-16 h-16" onClick={onTogglePlay} disabled={!isDeviceOnline}>
